...
crowns_sf_with_dbh %>% 
  ...
  dplyr::group_by(plotID,plot_area_m2) %>% 
  dplyr::mutate(
    height_rank = dplyr::row_number(desc(tree_height_m))
  ) %>% 
  dplyr::summarise(
    mean_dbh_cm = mean(dbh_cm, na.rm = T)
    , mean_tree_height_m = mean(tree_height_m, na.rm = T)
    , dominant_height_m = mean(ifelse(height_rank<=2,tree_height_m,NA), na.rm = T)
    , n_trees = dplyr::n_distinct(treeID)
    , basal_area_m2 = sum(basal_area_m2)
    # , basal_area_ft2 = sum(basal_area_ft2)
    , sum_dbh_cm_sq = sum(dbh_cm^2, na.rm = T)
  ) %>% 
  dplyr::mutate(
    trees_per_ha = (n_trees/plot_area_m2)*10000
    , basal_area_m2_per_ha = (basal_area_m2/plot_area_m2)*10000
    , qmd_cm = sqrt(sum_dbh_cm_sq/n_trees)
  ) %>% 
  dplyr::select(-c(sum_dbh_cm_sq)) %>% 
  dplyr::relocate(qmd_cm, .after = mean_dbh_cm) %>% 
  tidyr::pivot_longer(
    cols = -c(plotID)
    , names_to = "metric"
    , values_to = "value"
...
crop_crowns_by_id_and_colorize_stems_and_veg(config, las_grid, colorize_input_las, stem_color_hex_temp, veg_color_hex_temp, ground_color_hex_temp,
