...
```{r ex-var-window, include=FALSE, eval=TRUE}
library(tidyverse)
f_temp = function(x) {
  y = dplyr::case_when(
...
This function uses the CHM raster created in [this section](#create_chm) and applies the individual tree detection algorithm *using a variable window function of linear form* (with a minimum window size of 2m and a maximum window size of 5m)
...
```{r, include=F, eval=F}
# plot(master_chm, col = height.colors(50))
  # plot(sf::st_geometry(tree_tops), add = TRUE, pch = 20)
  # 
  xxx
