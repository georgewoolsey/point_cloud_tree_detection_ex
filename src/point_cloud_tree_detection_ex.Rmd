...
### Use the stems detected using TreeLS::treeMap to delineate crowns using MCWS from ForestTools
  ...
  crowns_treels = ForestTools::mcws(
...
ggplot() + 
  geom_tile(
    data = crowns_treels %>% as.data.frame(xy=T) %>% dplyr::rename(f=3)
    , mapping = aes(x=x,y=y,fill=as.factor(f))
  ) +
  geom_sf(
  ...
  scale_fill_manual(
    values = viridis::turbo(n = terra::freq(crowns_treels) %>% nrow()) %>% sample()
  ) +
  labs(x = "", y = "") +
  theme_light() +
  theme(legend.position = "none")
...
### Convert crown raster to terra rast, then polygons, then to Sf
crowns_treels_sf = crowns_treels %>% 
  # convert raster to polygons for each individual crown
  terra::as.polygons() %>% 
  # fix polygon validity
  terra::makeValid() %>% 
  ...
  # str(crowns_treels_sf)
  # ggplot(crowns_treels_sf) + geom_sf(aes(fill=as.factor(layer))) + 
  #   scale_fill_manual(values = viridis::turbo(nrow(crowns_treels_sf)) %>% sample()) +
  #   theme_void() + theme(legend.position = "none")
  
### Join the crowns_treels with the seeds to append data, remove Nulls
crowns_treels_sf = crowns_treels_sf %>%
  sf::st_join(dbh_locations_sf) %>% 
  dplyr::select(-c(layer))
  
  # str(crowns_treels_sf)
  # crowns_treels_sf %>% dplyr::count(treeID) %>% nrow()
  # crowns_treels_sf %>%
  #   sf::st_drop_geometry() %>%
  #   dplyr::summarise(
  #     n = n()
...
### Add crown data summaries
crown_sum_temp = data.frame(
    mean_crown_ht_m = terra::extract(x = master_chm, y = terra::vect(crowns_treels_sf), fun = "mean", na.rm = T)
    , median_crown_ht_m = terra::extract(x = master_chm, y = terra::vect(crowns_treels_sf), fun = "median", na.rm = T)
    , min_crown_ht_m = terra::extract(x = master_chm, y = terra::vect(crowns_treels_sf), fun = "min", na.rm = T)
  ) %>% 
  dplyr::select(-c(tidyselect::ends_with(".ID"))) %>% 
  dplyr::rename_with(~ stringr::str_remove_all(.x,".Z"))
...
### Write the crowns_treels to the disk
  sf::st_write(
    crowns_treels_sf
...
###________________________________________________________###
  ...
  crowns_sf_joined_stem_locations = crowns_sf %>%
    sf::st_join(
      dbh_locations_sf %>% 
  ...
  # str(crowns_sf_joined_stem_locations)
###________________________________________________________###
### Check for row uniqueness ###
###________________________________________________________###
  crowns_sf_joined_stem_locations %>% 
    sf::st_drop_geometry() %>% 
    dplyr::summarise(
      n = dplyr::n()
...
crowns_sf_joined_stem_locations = crowns_sf_joined_stem_locations %>% 
  dplyr::mutate(
  ...
  # summary(crowns_sf_joined_stem_locations$est_dbh_pct_diff)
# build training data set
dbh_training_dataset = crowns_sf_joined_stem_locations %>%
  sf::st_drop_geometry() %>% 
  dplyr::filter(
    !is.na(stem_dbh_cm)
...
# dbh_training_dataset %>% dplyr::glimpse()
```
...
### tuning RF model
  ...
  rf_tune_temp = randomForest::tuneRF(
    y = dbh_training_dataset$stem_dbh_cm
    , x = dbh_training_dataset %>% dplyr::select(-c(treeID,stem_dbh_cm))
    , stepFactor = 0.5
    , ntreeTry = 500
    , mtryStart = 0.5
    , improve = 0.01
...
ggplot() +
  ...
  scale_x_continuous(limits = c(0,max(dbh_training_dataset$stem_dbh_cm)*1.05)) +
  scale_y_continuous(limits = c(0,max(dbh_training_dataset$stem_dbh_cm)*1.05)) +
  labs(
    x = "SfM DBH (cm)"
...
```{r model-dbhs-rslt3}
data.frame(
  mae = Metrics::mae(dbh_training_dataset$stem_dbh_cm, stem_prediction_model$predicted)
  , mape = Metrics::mape(dbh_training_dataset$stem_dbh_cm, stem_prediction_model$predicted) %>% 
    scales::percent(accuracy = .01)
  , rmse = Metrics::rmse(dbh_training_dataset$stem_dbh_cm, stem_prediction_model$predicted)
) %>% 
  kableExtra::kbl(
...
crowns_sf_predict_only_temp = crowns_sf %>%
  ...
  dplyr::select(
    dbh_training_dataset %>% dplyr::select(-c(stem_dbh_cm)) %>% names()
  )
# get predicted dbh
...
# write the data to the disk
  sf::st_write(
    crowns_sf_with_dbh
    , paste0(config$delivery_spatial_dir, "/final_stem_locations.gpkg")
    , append = FALSE
    , quiet = TRUE
  )
```
