...
### Using TreeLS::treeMap                               ###
###_____________________________________________________###
# pass this function a file path of the normalized las with stems classified (i.e. TreeID and Stem columns present)
las_path_name = list.files(config$las_stem_dir, pattern = ".*\\.(laz|las)$", full.names = T)[1]
# write_stem_las_fn <- function(las_path_name) {
    ### Get the desired las file
    las_name = basename(las_path_name)
    tile_name = stringr::word(las_name, start = 1, sep = "\\.")
    
    ### See if the file has been generated
    path_to_check = paste0(config$stem_poly_tile_dir, "/", tile_name, ".parquet")
    does_file_exist = file.exists(path_to_check)
    # does_file_exist
    
    if(does_file_exist == TRUE){
      message("stem DBH polygon for grid number ", las_name, " already exists guy ... ")
      return(FALSE)
    }
    
    ### Read in the desired las file
    las_stem_tile = lidR::readLAS(las_path_name)
    # sf::st_crs(las_stem_tile)
    # plot(las_stem_tile, color = "Classification")
    las_stem_tile@data %>% dplyr::glimpse()
        
    ### Search through tree points and estimate DBH
    tree_inv = TreeLS::tlsInventory(
      las = las_stem_tile
      # height layer (above ground) to estimate stem diameters, in point cloud units
      , dh = 1.37
      # height layer width, in point cloud units
