...
### Set the minimum height for the stem-only canopy height model
min_stem_height_threshold = 60
...
<span style="color: red;">*George W. notes: this buffer is buffering each cell instead of buffering the extent and then creating the grid...which is it supposed to be for processing?*</span>
...
mapview::mapview(
  ...
  mapview::mapview(las_grid_buff, color = "gray55", alpha.regions = 0, lwd = 2, legend = F, label = F, hide = T) +
...
las2dtm2df <- function(file_path_name, des_dtm_res = 3) {
  ### Read in the file
  las_ground_tile = lidR::readLAS(file_path_name, select = "xyzc")
...
# GW added
# create funtion to read las, rasterize, and return data frame
las2chm2df <- function(file_path_name, des_chm_res = 3, min_z = -Inf, max_z = Inf) {
  ### Read in the file
  las_norm_tile = lidR::readLAS(file_path_name, select = "xyzc")
...
Lastly, points in the normalized point cloud are classified as stem points and assigned a tree ID using the `TreeLS::stemPoints` function with the algorithm `stm.hough`:
...
# detect_stems_within_tiles = function(config, dbh_max_size_m){
  ...
  message("Initializing tree stem detection algorithm ... ")
  master_start = Sys.time()
  
  ####____________________________________###
  ### Get a list of .las files to process ###
  ###_____________________________________###
  tile_list = list.files(config$las_norm_tile_dir, pattern = ".*\\.(laz|las)$", full.names = F)
  # tile_list
  ...
  ### select columns from the las extent
  las_extent = las_extent %>% 
    dplyr::select(filename, Number.of.point.records) %>% 
    dplyr::mutate(
      las_name = basename(filename)
      , extent_area = sf::st_area(las_extent) %>% as.numeric()
      , number_points = Number.of.point.records
    )
  # las_extent %>% str()
  ...
  # cluster = parallel::makeCluster(cores-1)
  # doParallel::registerDoParallel(cluster)
  # stem_detection_status_grid_list = foreach(
  #   i = 1:length(tile_list), 
  ...
  ## !!!!!! TURNED OFF PARALLEL B/C IT KILLS STUFF
    ...
    if(does_file_exist == TRUE){
      message("stem detect laz for grid number ", i, " already exists guy ... ")
    }
    
    ### Read in the desired las file
    las_path = paste0(config$las_norm_tile_dir, "/", las_name)
    las_norm_tile = lidR::readLAS(las_path)
    las_norm_tile = lidR::filter_duplicates(las_norm_tile)
    st_crs(las_norm_tile) = st_crs(las_grid)
    # plot(las_norm_tile)
    # get the maximum point height
    max_point_height = max(las_norm_tile@data$Z)
    ### Filter the extent polygon sf/data.frame for the tile
    tile_extent = las_extent %>% 
      dplyr::filter(filename == las_path) %>% 
      dplyr::mutate(
        las_area_m2 = extent_area
        , max_point_height = max_point_height
      )
    # tile_extent %>% str()
    ###____________________________________________________________###
  ...
  stem_points_list = foreach(i=1:length(stem_point_list),
                             ...
                             .packages = c("data.table", "lidR")) %dopar% {
                               ...
                               setwd(config$las_stem_dir)
                               des_las = lidR::readLAS(des_las_name)
                               des_las
...
```{r detect-stem-st2, webgl=FALSE}
# test tree detection function
...
### Classify stem points
  ...
  # las_cls_temp@data %>% dplyr::glimpse()
  # las_cls_temp@data %>% dplyr::count(Classification)
  # las_cls_temp@data %>% dplyr::count(Stem)
  # las_cls_temp@data %>% dplyr::count(TreeID) %>% nrow()
...
# plot only ground points and stem points
  ...
  #   plot(color = "Stem")
# plot only ground points and stem points
las_norm_tile_temp = lidR::filter_poi(las_norm_tile_temp, (Stem==TRUE | Classification==2))
plot3D::scatter3D(
  x = las_norm_tile_temp@data$X
  , y = las_norm_tile_temp@data$Y
  , z = las_norm_tile_temp@data$Z
  , colvar = las_norm_tile_temp@data$Stem
  , col = viridis::viridis(las_norm_tile_temp@data$Stem %>% unique() %>% length())
  , pch = 19, cex = 0.4
)
```
...
detect_crowns_top_down = function(config, desired_chm_res, window_size, minimum_height, min_crown_size_m2){
  ...
  desired_chm_name = paste0("chm_raster_", desired_chm_res, "m.tif")
...
crop_crowns_by_id_and_colorize_stems_and_veg = function(config, las_grid, colorize_input_las, stem_color_hex, veg_color_hex, ground_color_hex, use_bottom_up_crowns){
  ...
  las_stem_ctg = lidR::readLAScatalog(config$las_stem_dir)
  las_stem_ctg
...
rasterize_tiles_to_dtm(config, des_dtm_res = 3, calculate_extent = FALSE)
