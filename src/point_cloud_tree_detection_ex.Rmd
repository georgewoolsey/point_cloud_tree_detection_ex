...
fnm_temp  = list.files(input_las_dir, pattern = "\\.laz$", full.names = T)[1]
...
# list.files(config$las_ground_tile_dir, pattern = "\\.laz$", full.names = T) %>%
#   .[2:3] %>%
...
rasterize_tiles_to_dtm = function(
  ...
  start_time = Sys.time()
  # file list
  tile_list = list.files(config$las_ground_tile_dir, pattern = "\\.laz$", full.names = T)
...
# GW added
# create funtion to read las, rasterize, and return data frame
las2chm2df <- function(file_path_name, des_chm_res = 3) {
  ### Read in the file
...
## map files over the function
# list.files(config$las_norm_tile_dir, pattern = "\\.laz$", full.names = T) %>%
#   .[2:2] %>%
#   purrr::map(las2chm2df, des_chm_res = 1) %>%
#   dplyr::bind_rows() %>%
#   str()
...
rasterize_tiles_to_chm = function(
  ...
  start_time = Sys.time()
  # file list
  tile_list = list.files(config$las_norm_tile_dir, pattern = "\\.laz$", full.names = T)
  # map las2chm2df function
  merged_chm_points = tile_list %>%
    purrr::map(las2chm2df, des_chm_res = my_des_chm_res) %>%
    # .[2:3] %>%
    dplyr::bind_rows()
  ###_____________________________________###
  ### Make a las file from the CHM points ###
  ...
  outname = paste0(config$delivery_spatial_dir,"/master_chm_raster_", my_des_chm_res, "m.tif")
...
rasterize_tiles_to_chm(
  config = config
  , my_des_chm_res = 1 #0.25 #in meters
...
rasterize_tiles_to_stem_chm = function(config, des_stem_chm_res, min_z, max_z, calculate_extent){
  ...
  chm_point_list = foreach(i = 1:length(tile_list), .packages = c("lidR", "terra", "data.table", "sf")) %dopar% {
    ...
    las = lidR::readLAS(des_tile_name, select = "xyzc")
