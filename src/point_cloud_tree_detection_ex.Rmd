...
This section utilizes the Canopy Height Model (CHM) raster created in [this section](#create_chm) to perform individual tree detection from the top down (i.e. using raster cell height values). From [The `lidR` Package Book](https://r-lidar.github.io/lidRbook/itd-its.html#itd):
...
$$
\textsf{variable window radius} = \textsf{local maximum pixel} \times 0.1
$$

This particular variable window radius function has the shape:

```{r ex-window, include=FALSE, eval=TRUE}
f_temp = function(x){x*0.1}
data.frame(
  height_m = seq(0,max_height_threshold,2)
) %>% 
dplyr::mutate(
...
## Define the tree detection function

This function reads a CHM raster file stored as a `tif`and applies the individual tree detection algorithm.
...
ws_fn = function(x) {
  y = x * 0.1
  y = ifelse(is.na(x), 0.1, max(y,0.1))
  return(y)
}
###___________________________________________________###
### create function to detect crowns top down
###___________________________________________________###
### New function to detect crowns top down, independent from main pipeline
# list.files(config$delivery_spatial_dir, pattern = ".*chm_raster_z-Inf.*\\.tif$", full.names = T) 
tif_path_name = 
list.files(config$delivery_spatial_dir, pattern = ".*chm_raster_z-Inf.*\\.tif$", full.names = T)[1]

# detect_crowns_top_down = function(config, desired_chm_res, window_size, minimum_tree_height, min_crown_size_m2){

  ### Read in the chm
  input_chm = terra::rast(tif_path_name)
  # input_chm 
  # terra::crs(input_chm)
  # plot(input_chm)
  
  ### Get tree tops using liDR
  
  # ITD on CHM
  # call the locate trees function and pass the variable window
  input_seeds = lidR::locate_trees(
    input_chm
    , algorithm = lmf(
      ws = ws_fn
  ...
  # plot(input_chm, col = height.colors(50))
  # plot(sf::st_geometry(input_seeds), add = TRUE, pch = 20)
  # 
  xxx
