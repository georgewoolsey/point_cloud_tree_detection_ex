...
## Define the Write Stem `laz` function

This function applies the [stem detection function](#detect_stem_fn) to the normalized point cloud to i) assign tree IDs to the points and ii) flag the stem points. The result writes the output to the `r config$las_stem_dir` directory.

```{r write-stem-las-fn}
# pass this function a file path of the las you wish to detect stems and classify
write_stem_las_fn <- function(las_path_name) {
    ...
    if(does_file_exist == TRUE){
      message("stem detect laz for grid number ", las_name, " already exists guy ... ")
      return(FALSE)
    }
    ...
    if(max_point_height >= 2){
      ...
      if(check == "LAS"){
        ...
        ### Pull out the remaining points that arent ground
        remaining_points = filter_poi(las_norm_tile, Stem == FALSE)
        remaining_points@data$Classification = 5
...
```{r call-detect-stems, message=FALSE, warning=FALSE}
...
# DBH Estimates{#est_dbh}

This section uses the normalized point cloud created in the [Detect Stem section](#detect-stems) with points assigned to a tree ID (where a stem was detected) and points classified as stems to estimate the DBH.
