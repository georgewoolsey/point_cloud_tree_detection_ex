...
Attached are two scripts that feed into each other, along with the FIA data:
  
FIA Prediction Interval: this should take FIA data downloaded from the DataMart, filter it based on some user parameters, and then generate the prediction interval. We could look at using the rFIA package for R to streamline the data import, users would just define the state the data comes and it should just plug in for my read.csv.

Filter UAS DBHs with FIA Prediction Interval: This will filter the detected DBH and height data pairs and eliminate any DBH values that are outside the 90% prediction bounds.
 
I like the bounding statement you used for the variable search window for tree detection. I do think we probably want this in the user-defined section so that it is easy to adjust.

Looking at the MCWS for canopy delineation, Neal implements a minimum crown area of like 2 m^2, this seems stupid to me and I think we should just eliminate it.
```
...
# Model Study Area DBH-to-Height Relationship
...
# terra::freq(treemap_rast) %>% dplyr::pull(value)
# get tree list for 
treemap_trees_df = readr::read_csv(
  ...
  dplyr::rename_with(tolower) %>% 
  dplyr::filter(
    # keep treemap id in area of interest 
    tm_id %in% (terra::freq(treemap_rast) %>% dplyr::pull(value))
